...
const getHighLight = (text) => {
	return text.replaceAll('*s*', `<span class="text-bold text-red">`).replaceAll('*e*', '</span>');
};
...
<template>
	<view class="mb-4 mx-20 flex">
		<uni-search-bar placeholder="搜索文件" v-model="s" class="flex-1" cancelButton="none" bgColor="#ffffff" @confirm="goSearch"></uni-search-bar>
		<button size="mini" class="mx-2 my-12" type="info" @click="openDialog">筛选</button>
		<button size="mini" class="mx-0 my-12" type="info" @click="goSearch">搜索</button>
	</view>
	<uni-popup ref="popup" background-color="#fff" type="right" @maskClick="goSearch">
		...
		<view class="filter-popup">
			<uni-section title="考试时间" class="mb-10" type="line">
				<view class="flex items-center mb-8">
					...
					<uni-datetime-picker v-model="searchInfo.range[0]" type="date" :clear-icon="false" return-type="timestamp" />
				</view>
				<view class="flex items-center">
					<view class="mr-10">到</view>
					<uni-datetime-picker v-model="searchInfo.range[1]" type="date" :clear-icon="false" return-type="timestamp" />
				</view>
			</uni-section>
			<uni-section title="考试年级" class="mb-10" type="line">
				<uni-data-select v-model="searchInfo.grade" :localdata="getOptions(grades)"></uni-data-select>
			</uni-section>
			<uni-section title="考试科目" class="mb-10" type="line">
				<uni-data-checkbox v-model="searchInfo.courses" :multiple="true" :localdata="getOptions(courses)"></uni-data-checkbox>
			</uni-section>
		</view>
	</uni-popup>
	<view v-if="data">
		...
		<view v-for="item in data">
			<uni-card @click="gotoFile(item.exam.eid, item.paper.pid, item.fid)">
				<view class="flex justify-between">
					<view>
						<view class="flex gap-10">
							<text class="text-red">{{ item.union.name }}</text>
							<text class="text-black">{{ GetYearMonth(item.examgroup.date) }} {{ item.examgroup.name }}</text>
						</view>
						...
						<view class="text-bold text-lg text-black mb-2">{{ item.name }}</view>
					</view>
					<view class="text-right text-bold">
						...
						<view class="text-black text-base">{{ file_types[item.type] }}</view>
					</view>
				</view>
				<view v-html="getHighLight(t)" v-for="t in item.text.slice(0, 2)"></view>
			</uni-card>
		</view>
		<view class="mx-30">
